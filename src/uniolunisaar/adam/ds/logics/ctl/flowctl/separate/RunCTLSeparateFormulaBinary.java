 ...
 */
class RunCTLSeparateFormulaBinary extends FormulaBinary<IRunFormula, RunCTLOperators.Binary, IRunFormula> implements IRunFormula {

    public RunCTLSeparateFormulaBinary(IRunFormula phi1, RunCTLOperators.Binary op, IRunFormula phi2) {
        super(phi1, op, phi2);
    }
    ...
    public RunCTLSeparateFormula createSubstitutedFormula(IFormula subformula, IFormula with) throws NotSubstitutableException {
        ...
        if (phi1 instanceof IRunFormula) {
            if (phi2 instanceof IRunFormula) {
                return new RunCTLSeparateFormula((IRunFormula) phi1, getOp(), (IRunFormula) phi2);
            } else {
