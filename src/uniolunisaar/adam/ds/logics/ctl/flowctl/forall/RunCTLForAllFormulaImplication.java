 ...
 */
class RunCTLForAllFormulaImplication extends FormulaBinary<ILTLFormula, RunOperators.Implication, IRunFormula> implements IRunFormula {

    public RunCTLForAllFormulaImplication(ILTLFormula phi1, RunOperators.Implication op, IRunFormula phi2) {
        super(phi1, op, phi2);
    }

    @Override
    public RunCTLForAllFormulaImplication createSubstitutedFormula(IFormula subformula, IFormula with) throws NotSubstitutableException {
        ...
        if (phi1 instanceof ILTLFormula) {
            if (phi2 instanceof IRunFormula) {
                return new RunCTLForAllFormulaImplication((ILTLFormula) phi1, getOp(), (IRunFormula) phi2);
            } else {
