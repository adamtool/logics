...
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatBaseListener;
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatParser;
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatParser.CtlContext;
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatParser.FlowFormulaContext;
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatParser.LtlContext;
import uniolunisaar.adam.logic.parser.logics.flowctl.forall.antlr.FlowCTLForAllFormatParser.RunFormulaContext;

/**
 *
 * @author Manuel Gieseking
 */
public class FlowCTLForAllListener extends FlowCTLForAllFormatBaseListener {

    private final PetriNet net;
    private RunCTLForAllFormula formula;
    ...
    private final Map<RunFormulaContext, RunCTLForAllFormula> runFormulas = new HashMap<>();
    private final Map<FlowFormulaContext, FlowCTLFormula> flowFormulas = new HashMap<>();

    public FlowCTLForAllListener(PetriNet net) {
        this.net = net;
    }

    @Override
    public void exitFlowCTL(FlowCTLForAllFormatParser.FlowCTLContext ctx) {
        formula = runFormulas.get(ctx.runFormula());
    }

    @Override
    public void exitRunFormula(FlowCTLForAllFormatParser.RunFormulaContext ctx) {
        ...
        if (ctx.ltl() != null) {
            ...
            if (ctx.runBinary().op.rand() != null) {
                f = new RunCTLForAllFormula(runFormulas.get(ctx.runBinary().phi1), RunOperators.Binary.AND, runFormulas.get(ctx.runBinary().phi2));
            } else if (ctx.runBinary().op.ror() != null) {
                f = new RunCTLForAllFormula(runFormulas.get(ctx.runBinary().phi1), RunOperators.Binary.OR, runFormulas.get(ctx.runBinary().phi2));
...
//                f = new RunCTLNestedFormula(runFormulas.get(ctx.runBinary().phi1), RunCTLOperators.Binary.BIMP, runFormulas.get(ctx.runBinary().phi2));
            ...
            f = new RunCTLForAllFormula(flowFormulas.get(ctx.flowFormula()));
        }
        if (f != null) {
    ...
    @Override
    public void exitFlowFormula(FlowCTLForAllFormatParser.FlowFormulaContext ctx) {
...
//        }
    ...
    @Override
    public void exitCtl(FlowCTLForAllFormatParser.CtlContext ctx) {
        ICTLFormula f = null;
        if (ctx.atom() != null) {
            ...
            FlowCTLForAllFormatParser.CtlUnaryOpContext opCtx = ctx.ctlUnary().op;
            ...
            if (ctx.ctlBinary().stdOp != null) {
                FlowCTLForAllFormatParser.CtlBinaryOpContext opCtx = ctx.ctlBinary().stdOp;
                if (opCtx.and() != null) {
                ...
                FlowCTLForAllFormatParser.CtlBinaryTempOpContext opCtx = ctx.ctlBinary().op;
                if (opCtx.until() != null) {
...
//                }
    ...
    @Override
    public void exitLtl(FlowCTLForAllFormatParser.LtlContext ctx) {
        ILTLFormula f = null;
        if (ctx.atom() != null) {
            String id = ctx.atom().getText();
    ...
    public RunCTLForAllFormula getFormula() {
        return formula;
    }
